...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    if len(sys.argv) != 2:
        print('The program must be run as : python3.5 CNN6.py [train|eval]')
        sys.exit(2)
    else:
        if sys.argv[1] == 'train':
            print('Run Train .....')
...
if __name__ == "__main__":
    ...
    tf.app.flags.DEFINE_string('train_dir', 'result/CNN6/train_result',
                               """Directory where to write event logs and checkpoint.""")
    tf.app.flags.DEFINE_integer('max_steps', 100000, """Number of batches to run.""")
    tf.app.flags.DEFINE_boolean('log_device_placement', False, """Whether to log device placement.""")
    tf.app.flags.DEFINE_string('eval_dir', 'result/CNN6/eval_result', """Directory where to write event logs.""")
    tf.app.flags.DEFINE_string('eval_data', 'test', """Either 'test' or 'train_eval'.""")
    tf.app.flags.DEFINE_string('checkpoint_dir', 'result/CNN6/train_result',
